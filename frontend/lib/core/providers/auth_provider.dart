...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      // Clear any cached session to ensure fresh role data
      await _storage.delete(key: 'user_session');
      print('DEBUG: AuthProvider cleared cached session for fresh login');

      final user = await _authService.login(email, password);
      if (user != null) {
        _user = user;
        await _saveSession(user);
        print('DEBUG: AuthProvider login successful, user role: ${user.role}');
        return true;
      }
      return false;
    } catch (e) {
