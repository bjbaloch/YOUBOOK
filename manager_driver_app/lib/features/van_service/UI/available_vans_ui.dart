  ...
  Future<void> _checkSeats(VanServiceInfo vanService) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Seat layout not available in UI-only mode.')),
      );
    }
  }
...
class _SeatLayoutDisplayScreenState extends State<SeatLayoutDisplayScreen> {
  ...
  Future<void> _bookSelectedSeats() async {
    ...
    // TODO: Restore wallet check + booking API calls when connecting backend
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Booking not available in UI-only mode.')),
      );
    }
  }
