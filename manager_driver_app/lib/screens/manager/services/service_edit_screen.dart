...
class _ServiceEditScreenState extends State<ServiceEditScreen> {
  ...
  Future<void> _loadService() async {
    ...
    // For UI-only mode, initialize with empty/default values
    _selectedStatus = ServiceStatus.active;
  ...
  Future<void> _deleteService() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Service deleted! (UI-only mode)')),
      );
      Navigator.pop(context, true);
    }
  }
      ...
      children: [
        ...
        InkWell(
          onTap: () {
            ...
            if (isVanService) {
              seatLayoutScreen = const PlaceholderSeatLayoutScreen(title: 'Van Seat Layout');
            } else if (isBusService) {
              seatLayoutScreen = const PlaceholderSeatLayoutScreen(title: 'Bus Seat Layout');
            }
...
// Placeholder seat layout screen for missing components
class PlaceholderSeatLayoutScreen extends StatelessWidget {
  final String title;

  const PlaceholderSeatLayoutScreen({super.key, required this.title});
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Icon(Icons.event_seat, size: 80, color: cs.primary),
            ...
            Text(
              'Seat layout configuration coming soon...',
              style: TextStyle(
                fontSize: 16,
