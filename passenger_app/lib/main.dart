...
import 'screens/splash_screen/passenger_splash_screen.dart';
...
void main() async {
  ...
  print('DEBUG: Running PassengerApp...');
...
class _PassengerAppState extends State<PassengerApp> {
  ...
  void _handleDeepLink(Uri uri) async {
    ...
    if (uri.scheme == 'youbook' && uri.host == 'auth') {
      print(
        'DEBUG: Deep link is auth callback - processing email confirmation',
      );
      ...
      print(
        'DEBUG: - access_token: ${accessToken != null ? 'present (${accessToken.length} chars)' : 'null'}',
      );
      print(
        'DEBUG: - refresh_token: ${refreshToken != null ? 'present (${refreshToken.length} chars)' : 'null'}',
      );
      ...
      if (accessToken != null && refreshToken != null) {
        ...
        try {
          ...
          if (currentUser != null) {
            ...
            final nextScreen = PassengerHomeUI(data: PassengerHomeData());

            // Navigate to the appropriate screen - use a more reliable navigation method
            if (mounted) {
              print(
                'DEBUG: Performing navigation to ${nextScreen.runtimeType}',
              );
        ...
        print(
          'DEBUG: Missing access_token or refresh_token in query params - cannot process auth',
        );
      }
    } else {
      print(
        'DEBUG: Deep link is not auth callback (scheme: ${uri.scheme}, host: ${uri.host}), ignoring',
      );
    }
